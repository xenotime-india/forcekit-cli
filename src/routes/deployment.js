...
export default cli => {
  ...
  const help = new cli.help('deployment', cli);
  ...
  cli.program
    ...
    .action(() => {
      cli.runCommand(cli.commands.deployment.retrieve, {
        directory : cli.program.directory ? cli.program.directory : cli.program.zipFile ? undefined : './',
  ...
  help.add('deploy', function() {
    ...
    this.line('    -RT, --testLevel [testLevel]     Specifies which tests are run as part of a deployment (NoTestRun/RunSpecifiedTests/RunLocalTests/RunAllTestsInOrg).'.input);
    this.line('    -RT, --runTests [runTests]       A list of Apex tests to run during deployment (commma separated).'.input);
    this.line('    -IW, --ignoreWarnings            Indicates whether a warning should allow a deployment to complete successfully (true) or not (false).'.input);
    this.line('    -RB, --rollbackOnError           Indicates whether any failure causes a complete rollback (true) or not (false).'.input);
    this.line('    --pollTimeout [pollTimeout]      Polling timeout in millisec (default is 60000ms).'.input);
    this.line('    --pollInterval [pollInterval]    Polling interval in millisec (default is 5000ms).'.input);
    this.line('    -v, --verbose                    Output execution detail log.'.input);
  });
  ...
  cli.program
    ...
    .option('-TL, --testLevel [testLevel]', 'Specifies which tests are run as part of a deployment (NoTestRun/RunSpecifiedTests/RunLocalTests/RunAllTestsInOrg)')
    .option('-RT, --runTests [runTests]', 'A list of Apex tests to run during deployment (commma separated)', common.parseList)
    .option('-IW, --ignoreWarnings', 'Indicates whether a warning should allow a deployment to complete successfully (true) or not (false).')
    .option('-RB, --rollbackOnError', 'Indicates whether any failure causes a complete rollback (true) or not (false)')
    .option('--pollTimeout [pollTimeout]', 'Polling timeout in millisec (default is 60000ms)', parseInt)
    .option('--pollInterval [pollInterval]', 'Polling interval in millisec (default is 5000ms)', parseInt)
    .option('-v, --verbose', 'Output execution detail log')
